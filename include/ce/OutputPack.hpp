...
namespace ce
{
    ...
    template<class T>
    struct OutputParameterHandler<HashedOutput<T>, void, 9>
    {
    ...
    template<class T>
    struct OutputParameterHandler<T, typename std::enable_if<std::is_base_of<HashedBase, T>::value>::type, 9>
    {
        static constexpr const bool IS_OUTPUT = true;
        using result_storage_type = ct::VariadicTypedef<decay_t<T>>;
    ...
    struct OutputPack
    {
        static constexpr const size_t OUTPUT_COUNT = OutputPack<Args...>::OUTPUT_COUNT + (OutputParameterHandler<T>::IS_OUTPUT ? 1 : 0);
        using storage_type = typename OutputParameterHandler<T>::result_storage_type;
        using result_storage_types = typename ct::Append<storage_type, typename OutputPack<Args...>::result_storage_types>::type;
